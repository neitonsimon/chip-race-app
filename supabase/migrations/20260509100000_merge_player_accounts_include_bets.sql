-- migration: 20260509100000_merge_player_accounts_include_bets.sql
-- Objetivo: Atualizar a função de fusão de contas para incluir também as tabelas relacionadas a apostas (user_bets e bet_odds), evitando o erro de foreign key constraint.
...
BEGIN
  ...
  SET user_id = p_new_id, guest_name = NULL 
  WHERE user_id = p_old_id 
    AND NOT EXISTS (
      SELECT 1 FROM public.bet_odds new_bo 
      WHERE new_bo.bet_id = public.bet_odds.bet_id AND new_bo.user_id = p_new_id
    );
  DELETE FROM public.bet_odds WHERE user_id = p_old_id;

  UPDATE public.user_bets SET user_id = p_new_id WHERE user_id = p_old_id;
  UPDATE public.user_bets SET punter_id = p_new_id WHERE punter_id = p_old_id;
  UPDATE public.user_bets SET created_by = p_new_id WHERE created_by = p_old_id;
  ...
  -- 1. Update JSON results in Events Table
