...
export const Hero: React.FC<HeroProps> = ({ 
  ...
  const renderMonthCard = (month: MonthData, index: number) => {
      ...
      return (
        <div key={index} className={`relative pt-6 pb-3 px-2 rounded-xl border flex flex-col items-center justify-center transition-all duration-300 min-h-[160px] ${
          isActive 
          ...
          {/* Status Badge */}
          <div className={`absolute top-0 left-0 w-full text-[9px] uppercase font-black py-1 tracking-widest rounded-t-lg ${
          ...
          <div className={`mt-3 w-full flex justify-center`}>
            {isAdmin ? (
                ...
                <div className={`text-xs py-1 px-2 rounded-full font-bold ${
                  isActive ? 'bg-primary text-white' : 
          ...
          {isAdmin && (
              <button 
                  ...
                  className={`absolute bottom-1 right-1 w-6 h-6 rounded-full flex items-center justify-center border z-20 ${
                      isActive ? 'bg-green-500 border-green-300' : 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button 
            ...
            className="group relative inline-flex items-center justify-center px-8 py-4 border border-transparent text-lg font-bold rounded-full text-white bg-gradient-to-r from-primary to-accent hover:from-accent hover:to-primary shadow-neon-pink transition-all duration-300 hover:scale-105"
          >
             <EditableContent 
                isAdmin={isAdmin}
                value={content.btn_ranking}
                onSave={(val) => onUpdateContent('btn_ranking', val)}
                className="text-white"
            />
          </button>
          <button 
            onClick={() => onNavigate('the-chosen-details')}
            className="group relative inline-flex items-center justify-center px-8 py-4 border border-gray-300 dark:border-white/20 text-lg font-bold rounded-full text-gray-700 dark:text-white hover:bg-white/10 transition-all duration-300 backdrop-blur-sm"
          >
            <EditableContent 
                isAdmin={isAdmin}
                value={content.btn_details}
                onSave={(val) => onUpdateContent('btn_details', val)}
            /> 
            <span className="ml-1">{prizeLabel}</span>
          </button>
        </div>
