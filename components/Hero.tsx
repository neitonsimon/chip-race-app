...
export const Hero: React.FC<HeroProps> = ({
  ...
  const totalSlides = activeEventSlides.length + 2; // +1 Bet, +1 The Chosen
  ...
  const isBetSlide = currentSlide === activeEventSlides.length;
    ...
    <div className="relative pt-20 pb-0 lg:pt-28 lg:pb-0 overflow-hidden min-h-[500px]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
        ...
        <div className="relative w-full mb-12 min-h-[300px] flex items-center justify-center">
          ...
          {/* Bet Slide */}
          <div className={`w-full transition-opacity duration-1000 ${isBetSlide ? 'opacity-100 relative z-10' : 'opacity-0 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none'}`}>
            <h1 className="text-4xl sm:text-6xl lg:text-8xl font-display font-black text-white mb-6 leading-tight">
              CHIP RACE BET<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-red-600 text-glow">
                MERCADO DE APOSTAS
              </span>
            </h1>
            <p className="mt-4 text-lg sm:text-2xl text-gray-300 max-w-3xl mx-auto font-light leading-relaxed">
              Dê seus palpites nos seus jogadores favoritos e multiplique suas fichas!
            </p>
            <div className="flex justify-center mt-12 mb-4">
              <button
                onClick={() => onNavigate('bet')}
                className="group relative bg-gradient-to-r from-red-500 via-rose-600 to-red-700 text-white font-black py-3 px-6 sm:py-4 sm:px-10 rounded-2xl shadow-[0_0_30px_rgba(239,68,68,0.3)] hover:shadow-[0_0_50px_rgba(239,68,68,0.6)] transition-all duration-500 transform hover:-translate-y-2 flex items-center gap-2 sm:gap-4 uppercase tracking-[0.2em] text-xs sm:text-sm overflow-hidden cursor-pointer"
              >
                <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700 skew-x-12" />
                <span className="material-icons-outlined text-xl sm:text-2xl animate-pulse text-red-300">sports_esports</span>
                <span className="relative z-10">APOSTAR AGORA</span>
                <span className="material-icons group-hover:translate-x-2 transition-transform relative z-10">arrow_forward</span>
              </button>
            </div>
          </div>
        ...
        <div className="flex justify-center gap-3 mb-10">
          ...
          <button
            ...
            className={`h-3 rounded-full transition-all duration-300 ${isBetSlide ? 'w-8 bg-red-500 scale-110 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'w-3 bg-white/20 hover:bg-white/40'}`}
            aria-label="Slide Bet"
          />
