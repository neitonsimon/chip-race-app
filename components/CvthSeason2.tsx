...
export const CvthSeason2: React.FC<CvthSeason2Props> = ({ isAdmin = false, onNavigate }) => {
  ...
  const handleAddQualifier = async () => {
    ...
    const newConfig = { ...config, qualifiers: newQualifiers };
    ...
    if (isAdmin) {
      ...
      try {
        const { error } = await supabase
          .from('content_db')
          .upsert({ key: 'cvth_season2', value: newConfig });
  ...
  const handleDeleteQualifier = async (id: string) => {
    const newQualifiers = qualifiers.filter(q => q.id !== id);
    const newConfig = { ...config, qualifiers: newQualifiers };
